<?php
    ...
    {   
        ...
        function Header(){             
            ...
            $this->Cell(170, 5, "FACTURA COMPRA", 0,1, 'R', 0);      
            ...
            $sql=pg_query("select F.id_factura_compra,F.id_factura_compra,F.fecha_actual,F.hora_actual,F.numero_serie,F.numero_autorizacion,F.fecha_cancelacion,P.empresa,P.nombres_completos,F.id_forma_pago from factura_compra F, proveedor P where F.id_proveedor=P.id_proveedor and id_factura_compra='$_GET[id]'");    
    ...
    $total = 0;      
    $iva_base = 1.12;
    $sql=pg_query("select D.cantidad,P.descripcion,D.precio,D.total from factura_compra F,detalle_factura_compra D,productos P where F.id_factura_compra=D.id_factura_compra and D.id_productos=P.id_productos and D.id_factura_compra='$_GET[id]' and P.incluye_iva = 'No'");   
    while($row=pg_fetch_row($sql)){                
        $pdf->SetX(1);                  
    ...
    $sql=pg_query("select D.cantidad,P.descripcion,D.precio,D.total from factura_compra F,detalle_factura_compra D,productos P where F.id_factura_compra=D.id_factura_compra and D.id_productos=P.id_productos and D.id_factura_compra='$_GET[id]' and P.incluye_iva = 'Si'");   
    while($row=pg_fetch_row($sql)){                
        $total_si = 0;
        $total_sit = 0;
