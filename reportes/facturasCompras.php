<?php
    ...
    while($row=pg_fetch_row($consulta)) {
        $consulta1=pg_query("select F.numero_serie,F.fecha_actual,F.hora_actual,F.fecha_cancelacion,F.numero_autorizacion,O.descripcion,F.tarifa0,F.tarifa12,F.iva,F.descuento,F.total,P.empresa,P.identificacion,P.nombres_completos,F.id_factura_compra from factura_compra F,proveedor P, formas_pago O where F.id_forma_pago = O.id_forma_pago and F.id_proveedor=P.id_proveedor and F.id_proveedor='$row[0]' and fecha_actual between '$_GET[inicio]' and '$_GET[fin]' order by F.id_factura_compra asc");
