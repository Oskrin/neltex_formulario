<?php
    ...
    class PDF extends FPDF {   
        ...
        function Header() {                         
            ...
            $this->Cell(190, 5, utf8_decode("RESUMEN GENERAL DE FACTURAS"),0,1, 'C',0);                                                                                                                            
            $this->SetFont('Amble-Regular','',10);        
            $this->Ln(3);
    ...
    $pdf->Cell(46, 6, utf8_decode('Nro Factura'),1,0, 'C',0);                                     
    ...
    $pdf->Cell(24, 6, utf8_decode('Tipo Pago'),1,1, 'C',0); 
                                                                         
    $consulta1=pg_query("select F.numero_serie,F.fecha_actual,F.hora_actual,F.fecha_cancelacion,F.tipo_precio,O.descripcion,F.tarifa0,F.tarifa12,F.iva,F.descuento,F.total,C.identificacion,C.nombres_completos,F.id_factura_venta,F.estado from factura_venta F, cliente C, usuario U, formas_pago O where F.id_forma_pago = O.id_forma_pago and F.id_cliente=C.id_cliente and U.id_usuario=F.id_usuario  order by F.id_factura_venta asc");
    while($row1=pg_fetch_row($consulta1)) { 
        if($row1[14] == "Activo") {       
            ...
            $pdf->Cell(46, 6, utf8_decode(substr($row1[0],8)),0,0, 'C',0);                                
